...
This report is the A* cell of **Experiment A**: the same centered, normalized-\(\mathcal Q\), equal-weight canonical-query bank used by V2.10. Protocol: [`EXPERIMENT_A_CENTERED_Q_PROBES.md`](../protocols/EXPERIMENT_A_CENTERED_Q_PROBES.md).
...
In particular, `medium_diagonal` moved from about \(-0.0747\) under Dijkstra to essentially zero under A* (\(+0.0003\)). That fails the first persistence check for a durable favorable mechanism corridor. The safer interpretation is:

> The strong Dijkstra category effect may reflect the interaction between the centered query, the actuator-cost basin, and uninformed search expansion rather than an intrinsic task-direction advantage.

Experiment A remains a centered canonical-probe study. It is not a random task population, uniform \(\mathcal Q\) sampling, Cartesian task sampling, or evidence about an entire diagonal region. See [`PROJECT_NOTE_EXPERIMENT_A_TASK_SET_EFFECT.md`](../../architecture/notes/PROJECT_NOTE_EXPERIMENT_A_TASK_SET_EFFECT.md).
...
- Out of scope (unchanged): weighted/anytime A*, bidirectional search, PRM/RRT/OMPL, bank regeneration, \(W>1\), 3R, new objectives, Experiment B Cartesian goal regions.
