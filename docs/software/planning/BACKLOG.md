...
- **V3-DEFER-001 — Deferred native planner breadth:** the narrowed V3.3/V3.4 vertical slices delivered eight-connected Dijkstra/A*, basic PRM, and RRTConnect. Weighted/bidirectional/any-angle lattice search, richer-than-eight connectivity, Lazy PRM, PRM*, plain RRT, RRT*, and informed/batch native planners remain accepted roadmap work but are intentionally deferred. Revisit after the V3.9 cross-DOF free-space closeout identifies which native implementations add scientific value rather than duplicate external-library coverage. See [V3 project plan](../V3_PROJECT_PLAN.md) §16 and [Sprint V3.4](sprints/v3/SPRINT_V3_4_NATIVE_ROADMAP_TREE.md).
- **V3-DEFER-002 — Spatial 4R/5R partial-task studies:** position-plus-pointing, orientation-cone, constrained-plane, remote-center, and other partial-pose tasks remain scientifically valuable, but they are no longer a gate between 3R and the standard 6R free-space architecture test. Revisit after V3.9 or when a partial-task paper question requires them. Any activation must preserve exact physical starts and task predicates appropriate to the available DOFs rather than forcing arbitrary `SE(3)` goals.
...
- **Experiment B exact-start control:** nearest-node start attachment is retained by the V2B-005 start-attachment decision (`retain_nearest_node_v1`) as a V2 formulation artifact; V3 removes `start_tolerance` as task semantics (ADR-023 accepted).
...
- **Sprint V2.7 (3R):** remains a frozen Version 2 proposal and is not the implementation path for higher DOF. New 3R work belongs to Sprint V3.7 under the V3 physical-state/task/planner contracts. See [sprints/v2/SPRINT_V2_7_3R_EXTENSION.md](sprints/v2/SPRINT_V2_7_3R_EXTENSION.md).
