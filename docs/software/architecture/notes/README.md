# Architecture notes

Design notes preserve useful explorations and deferred alternatives. Promote a note into an ADR or contract before treating it as normative implementation authority.

- [Future search algorithms](PROJECT_NOTE_FUTURE_SEARCH_ALGORITHMS.md)
- [Experiment A task-set effect](PROJECT_NOTE_EXPERIMENT_A_TASK_SET_EFFECT.md) (deferred; low confidence)
- [Experiment B crossed statistics](PROJECT_NOTE_EXPERIMENT_B_CROSSED_STATISTICS.md) (V2.12 prerequisite placeholder; production held under V3)
- [V2 to V3 pivot](PROJECT_NOTE_V2_TO_V3_PIVOT.md) (rationale; not an evidence claim)
- [V3 code inventory](V3_CODE_INVENTORY.md) (Sprint V3.0)
- [V3 migration map](V3_MIGRATION_MAP.md) (Sprint V3.0)
- [V3.6A dimensional-generalization migration](V3_6A_DIMENSIONAL_GENERALIZATION_MIGRATION.md) (Sprint V3.6A)
- [V3.6B Gate B review findings](V3_6B_GATE_B_REVIEW_FINDINGS.md) (Sprint V3.6B; V3.6C closeout accepted)
- [V3.6C closeout decisions](V3_6C_CLOSEOUT_DECISIONS.md) (Sprint V3.6C)
- [V3.6C Gate A corrective review](V3_6C_GATE_A_CORRECTIVE_REVIEW.md) (Sprint V3.6C; Gate A/B accepted)
- [V4.0 kinematic geometry core closeout](V4_0_KINEMATIC_GEOMETRY_CORE_CLOSEOUT.md) (Sprint V4.0)
- [V4.1 planar-2R intrinsic geometry atlas closeout](V4_1_PLANAR2R_GEOMETRY_ATLAS_CLOSEOUT.md) (Sprint V4.1)
- [Static wrench kinematic geometry method](STATIC_WRENCH_KINEMATIC_GEOMETRY_METHOD.md) (drafted V3.6E–F contract; unauthorized; consume V4.0 kernel)
- [S4-12 lifted output-state](S4-12-lifted-output-state.md)
