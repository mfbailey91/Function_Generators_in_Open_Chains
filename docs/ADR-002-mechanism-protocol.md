...
| `valid_input(u)` | Assembly / kinematic domain only. Does **not** apply shared output joint limits (IM-009). |
...
Concrete types register with `register_mechanism_type(type_key, cls)`. Dicts must include `"type"` matching a registered key. No pydantic models on this path; experiment configs remain separate (IM-014).
