#!/usr/bin/env python3
"""Generate the Sprint V3.7 planar 3R free-space review artifact.

Publication is intentionally two-stage. Commit the implementation first, then
run this script from a clean worktree. The manifest records that clean HEAD;
generated result files are committed in a second evidence commit.
"""
...
from inequality_mechanisms.benchmarks.free_space_report_3r import (
    build_html_v3_7,
    build_readme_v3_7,
    summarize_v3_7,
)
...
DEFAULT_OUTPUT = Path("results/v3_review/v3_7_3r_free_space")
DEFAULT_HTML_COPY = Path(
    "docs/software/experiments/reports/V3_7_3R_FREE_SPACE_EVIDENCE.html"
)
...
def main() -> None:
    ...
    if _git_dirty() and not args.allow_dirty:
        raise SystemExit(
            "Refusing V3.7 evidence generation from a dirty worktree. "
            "Commit the implementation first, then rerun. "
    ...
    rows, resolved_bank = run_free_space_evidence_3r(
        contract=contract,
        ompl_solve_time_s=args.ompl_solve_time_s,
    )
    manifest = evidence_manifest_3r(
        rows,
        contract=contract,
    ...
    (args.out / "README.md").write_text(
        build_readme_v3_7(manifest=manifest, summary=summary),
        encoding="utf-8",
    )
    html = build_html_v3_7(manifest=manifest, summary=summary)
    html_path = args.out / "V3_7_3R_FREE_SPACE_EVIDENCE.html"
    ...
    print(
        f"wrote V3.7 evidence rows={len(rows)} "
