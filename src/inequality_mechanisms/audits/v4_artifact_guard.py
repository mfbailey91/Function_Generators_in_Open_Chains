...
def allowed_v4_2_output_root() -> Path:
    """Absolute allowed V4.2 span atlas output root (may be monkeypatched)."""
...
def allowed_v4_2b_output_root() -> Path:
    """Absolute allowed V4.2B corrective output root (may be monkeypatched)."""
...
def allowed_v4_2c_r_output_root() -> Path:
    """Absolute allowed V4.2C-R clarification output root (may be monkeypatched)."""
...
def assert_v4_2_output_allowed(path: Path) -> Path:
    """Resolve ``path`` and assert it is under the V4.2 span-atlas output root.

    V4.2 writers must reject the retained V4.0 smoke package, the frozen
    V4.1 atlas, every V3 review package, sibling V4 packages, and
    arbitrary paths.
    """
    resolved = Path(path).expanduser().resolve()
    allowed = allowed_v4_2_output_root()
    if _is_under(resolved, allowed):
        return resolved

    _refuse_v3(resolved, writer="V4.2", allowed=allowed)

    v4_package = _v4_review_package_name(resolved)
    if v4_package == V4_0_ALLOWED_PACKAGE:
        raise ArtifactPathForbiddenError(
            "Refusing to write into frozen V4.0 retained evidence "
            f"at {resolved}. V4.2 may write only under {allowed}."
        )
    ...
    if v4_package is not None and v4_package != V4_2_ALLOWED_PACKAGE:
        raise ArtifactPathForbiddenError(
            f"Refusing to write into unauthorized V4 package {v4_package!r} "
            f"at {resolved}. V4.2 may write only under {allowed}."
        )
    ...
    raise ArtifactPathForbiddenError(
        f"V4.2 output path {resolved} is not under the allowed root {allowed}."
    )
...
def assert_v4_2b_output_allowed(path: Path) -> Path:
    """Resolve ``path`` and assert it is under the V4.2B corrective output root.
    ...
    _refuse_v3(resolved, writer="V4.2B", allowed=allowed)
...
def assert_v4_2c_r_output_allowed(path: Path) -> Path:
    """Resolve ``path`` and assert it is under the V4.2C-R clarification root.

    V4.2C-R writers read the frozen V4.2C package and write only the sibling
    clarification root. They must reject V4.0–V4.2C overwrite, every V3
    review package, ``v4_3_intrinsic_static_wrench``, and arbitrary paths.
    """
    resolved = Path(path).expanduser().resolve()
    allowed = allowed_v4_2c_r_output_root()
    if _is_under(resolved, allowed):
        return resolved

    _refuse_v3(resolved, writer="V4.2C-R", allowed=allowed)
    ...
    if v4_package in frozen:
        raise ArtifactPathForbiddenError(
            ...
            f"at {resolved}. V4.2C-R may write only under {allowed}."
        )
    if v4_package is not None and v4_package != V4_2C_R_ALLOWED_PACKAGE:
        raise ArtifactPathForbiddenError(
            f"Refusing to write into unauthorized V4 package {v4_package!r} "
            f"at {resolved}. V4.2C-R may write only under {allowed}."
        )
    ...
    raise ArtifactPathForbiddenError(
        f"V4.2C-R output path {resolved} is not under the allowed root {allowed}."
    )
...
def v4_1_atlas_package_digest() -> tuple[str, int]:
    """Digest git-tracked files of the retained V4.1 atlas package."""
    prefix = f"{V4_1_ALLOWED_OUTPUT_REL.as_posix()}/"
    paths = [
        rel
        for rel in git_ls_files(V4_1_ALLOWED_OUTPUT_REL.as_posix())
        if rel.startswith(prefix) or rel == V4_1_ALLOWED_OUTPUT_REL.as_posix()
    ]
    return digest_git_tracked_paths(paths)
